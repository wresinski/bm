...
int searchFile(char *path, char *t)
{
    ...
    while(fgets(s,1024,fp)!=NULL){
        ...
        if(c>0&&line_p==1) printf(RED "\n%s:\n" NONE,path);
        if(c>0) printf(YELLOW "%d:" NONE "%s",line,r);
    }
    fclose(fp);
    return 1;
}
...
*/
    ...
    if(argc<2) return 0;
    struct stat statbuf;
    if(lstat(argv[2],&statbuf)!=0) return 0;
    if(S_ISDIR(statbuf.st_mode)) searchDir(argv[2],argv[1]);
    else searchFile(argv[2],argv[1]);
    if(g){
        free(g);
        g = NULL;
        size = 0;
